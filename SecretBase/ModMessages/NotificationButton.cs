...
namespace SecretBase.ModMessages
{
	...
	/// 
	/// It's a pulsing icon of a blueberry patched into Cursors from WorldEditor
	/// that shows up when there are pending notifications in ModEntry from
	/// SMAPI mod messages sent between Secret Base owners and guests.
	/// 
	...
	///
	/// Opens up an instance of NotificationMenu when clicked.
	/// </summary>
	internal class NotificationButton : IClickableMenu
	{
		...
		{
			var sourceRect = new Rectangle(Editors.WorldEditor.IconLocation.X, Editors.WorldEditor.IconLocation.Y, 11, 14);
		...
		{
			return Button.containsPoint(x, y) && ModEntry.Instance.PendingNotifications.Count > 0;
		}
